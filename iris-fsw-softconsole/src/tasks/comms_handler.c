...
void commsTransmitterTask() {
  ...
  for (;;) {
    ...
    if (xQueueReceive(commsTxQueue, &packet, portMAX_DELAY) == pdTRUE) {
      vTaskSuspendAll();
      custom_MSS_UART_polled_tx_string(&g_mss_uart0, "[ ", 2);
			//pad header string to 16 bytes
      custom_MSS_UART_polled_tx_string(&g_mss_uart0,
                                       (const uint8_t*)&packet->headerStr,
                                       strlen((char*)packet->headerStr));
      custom_MSS_UART_polled_tx_string(&g_mss_uart0, "] - ", 4);
      custom_MSS_UART_polled_tx_string(
          &g_mss_uart0, (const uint8_t*)&packet->data, packet->len);
      vPortFree(packet);
      xTaskResumeAll();
    }
  }
