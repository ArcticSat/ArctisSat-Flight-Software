...
# Dynamically scale UI positions and sizes
indicator_corner_anchor_x = int(width * 0.488)
indicator_corner_anchor_y = int(height * 0.302)
voltage_anchor_x = int(width * 0.488)
current_anchor_x = int(width * 0.488)
voltage_anchor_y = int(height * 0.435)
current_anchor_y = int(height * 0.464)
GUI_loop_rate_anchor_x = int(width * 0.293)
GUI_loop_rate_anchor_y = int(height * 0.016)
...
font = pygame.font.Font(None, int(0.013*width))
small_font = pygame.font.Font(None, int(0.00781*width))
...
def updateAnchors():
    global width, height, indicator_corner_anchor_x, indicator_corner_anchor_y, voltage_anchor_x, current_anchor_x
    global voltage_anchor_y, current_anchor_y, GUI_loop_rate_anchor_x, GUI_loop_rate_anchor_y
    indicator_corner_anchor_x = int(width * 0.488)
    indicator_corner_anchor_y = int(height * 0.302)
    voltage_anchor_x = int(width * 0.488)
    current_anchor_x = int(width * 0.488)
...
def updateStatus():
    ...
    window.blit(power_label, (indicator_corner_anchor_x + 0.0162*width, indicator_corner_anchor_y + 0.0026*height))
    window.blit(adcs_label, (indicator_corner_anchor_x + 0.0162*width, indicator_corner_anchor_y + 0.0286*height))
    window.blit(comms_label, (indicator_corner_anchor_x + 0.0162*width, indicator_corner_anchor_y + 0.0547*height))
...
def drawVoltageAndCurrent(voltage, current):
    ...
    window.blit(voltage_num, (voltage_anchor_x + 90*(width/old_width), voltage_anchor_y))
    window.blit(current_num, (current_anchor_x + 90*(width/old_width), current_anchor_y))
...
def drawLoopRate(set_GUI_fps, current_fps):
    ...
    window.blit(GUI_loop_rate_num, (GUI_loop_rate_anchor_x + 130*(width/old_width), GUI_loop_rate_anchor_y))

def drawTextToScreen(received_data):
    # Draw the text window
    pygame.draw.rect(window, BLACK, text_window, 2)
    y_offset = text_window.y + 0.0052*height
    if(len(received_data) > max_lines):
        received_data.pop(0)
    for line in received_data:
        try:
            text_surface = font.render(line, True, BLACK)
            window.blit(text_surface, (text_window.x + 0.00326*width, y_offset))
...
class Button:
    def __init__(self, x, y, width_ratio, height_ratio, color, message, text, action = None, group = None):
        self.rect = pygame.Rect(int(x * width), int(y * height), int(width * width_ratio), int(height*height_ratio))
        self.color = color
    ...
    def draw(self, surface):
        ...
        font = pygame.font.Font(None, int(height * 0.02))  # Scale font size
...
class indicator:
    def __init__(self, x, y, color):
        self.rect = pygame.Rect(x, y, 0.013*width, 0.0208*height)
        self.color = color
...
class label:
    def __init__(self, x, y, text, font_size=int(0.013*width)):
...
class ScrollableTextbox:
    def __init__(self, x, y, box_width, box_height, color, font):
        ...
        self.checkbox = Checkbox(x + box_width - 0.013*width, y, 0.0208*height, "Snap", font)
        self.clearbox = Checkbox(x + box_width - 0.013*width, y + 0.0208*height, 0.0208*height, "Clear", font)
    ...
    def draw(self, surface):
        ...
        for i in range(self.scroll_offset, min(len(self.text_lines), self.scroll_offset + self.max_lines)):
            ...
            surface.blit(text_surface, (self.rect.x + 0.5*0.00651*width, y_offset))
        ...
        if len(self.text_lines) > self.max_lines:
            ...
            scrollbar_rect = pygame.Rect(self.rect.right - 0.00651*width, scrollbar_y, 0.0104*height, scrollbar_height)
...
class Checkbox:
    ...
    def draw(self, surface):
        ...
        text_surface = self.font.render(self.text, True, BLACK)
        surface.blit(text_surface, (self.rect.right + 0.00651*width, self.rect.centery - text_surface.get_height() // 2))
...
buttons = [
    Button(0.0326, 0.083, 0.26, 0.052, RED, "bruh", "ARM", armActuators, "ARMING"),
    Button(0.0326, 0.1458, 0.065, 0.052, RED, bytearray(b'\x01\x08\x00'), "Left Out", None, "ACTUATORS"),
    Button(0.0326, 0.208, 0.065, 0.052, GREEN, bytearray(b'\x01\x0A\x00'), "Left In", None, "ACTUATORS"),
    Button(0.228, 0.1458, 0.065, 0.052, RED, bytearray(b'\x01\x0C\x00'), "Both Out", None, "ACTUATORS"),
    Button(0.13, 0.1458, 0.065, 0.052, RED, bytearray(b'\x01\x09\x00'), "Right Out", None, "ACTUATORS"),
    Button(0.13, 0.208, 0.065, 0.052, GREEN, bytearray(b'\x01\x0B\x00'), "Right In", None, "ACTUATORS"),
    Button(0.228, 0.208, 0.065, 0.052, GREEN, bytearray(b'\x01\x0D\x00'), "Both In", None, "ACTUATORS"),
    Button(0.618, 0.083, 0.065, 0.052, PURPLE, bytearray(b'\x00\x00'), "Ping Power", None, "POWER"),
    Button(0.716, 0.083, 0.065, 0.052, PURPLE, bytearray(b'\x00\x01\x01'), "Get V and C", None, "POWER"),
    Button(0.52, 0.083, 0.065, 0.052, GREY, bytearray(b'\x01\x01\x02'), "Sync", None, "ADCS"),
    Button(0.326, 0.083, 0.065, 0.052, GREY, bytearray(b'\x01\x02\x0D'), "Gyro", None, "ADCS"),
    Button(0.326, 0.1458, 0.065, 0.052, GREY, bytearray(b'\x01\x03\x0B'), "Mag", None, "ADCS"),
    Button(0.326, 0.208, 0.065, 0.052, GREY, bytearray(b'\x01\x07\x03'), "Temp", None, "ADCS"),
    Button(0.423, 0.083, 0.065, 0.052, GREY, bytearray(b'\x01\x04\x15'), "Lat/Long/Alt", None, "ADCS"),
    Button(0.423, 0.1458, 0.065, 0.052, GREY, bytearray(b'\x01\x05\x11'), "Time", None, "ADCS"),
    Button(0.423, 0.208, 0.065, 0.052, GREY, bytearray(b'\x01\x06\x05'), "Num Sats", None, "ADCS"),
    Button(0.52, 0.1458, 0.065, 0.052, GREY, bytearray(b'\x01\x0E\x00'), "Reset GNSS", None, "ADCS"),
    Button(0.52, 0.208, 0.065, 0.052, GREY, bytearray(b'\x01\x0F\x02'), "Get data rate", None, "ADCS"),
    Button(0.651, 0.1458, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x02\x01'), "ON", None, "CCLSM"),#CDH
    Button(0.684, 0.1458, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x02\x00'), "OFF", None, "CCLSM"),
    Button(0.716, 0.1458, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x02\x03'), "DATA", None, "CCLSM"),
    Button(0.651, 0.208, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x01\x01'), "ON", None, "CCLSM"), #ADCS
    Button(0.684, 0.208, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x01\x00'), "OFF", None, "CCLSM"),
    Button(0.716, 0.208, 0.0326, 0.052, GREY, bytearray(b'\x00\x04\x01\x02'), "DATA", None, "CCLSM"),
    Button(0.651, 0.271, 0.0326, 0.052, GREY, bytearray(b'\x01\x12\x00'), "ON", None, "CCLSM"), #CAMERA
    Button(0.684, 0.271, 0.0326, 0.052, GREY, bytearray(b'\x01\x11\x00'), "OFF", None, "CCLSM"),
    Button(0.553, 0.5, 0.0488, 0.156, GREEN, bytearray(b'\x02\x01'), "Take\nImage", None, "BALLIN"),
    Button(0.553, 0.677, 0.0488, 0.156, GREEN, bytearray(b'\x02\x02'), "Downlink\nImage", None, "BALLIN"),
    Button(0.618, 0.359, 0.0456, 0.0781, GREEN, bytearray(b'\x99\x99'), "Format\nFS", None, "BALLIN"),
    Button(0.677, 0.359, 0.0456, 0.0781, GREEN, bytearray(b'\x02\x03'), "Get\nCDH\ntime", None, "BALLIN"),
    Button(0.736, 0.359, 0.0456, 0.0781, GREEN, bytearray(b'\x02\x05'), "Set\nCDH\ntime", sendTime, "BALLIN"),
]

get_telem_button = Button(0.371, 0.594, 0.0488, 0.106, GREEN, bytearray(b'\x08\x08'), "Get\nTelem", None, "BALLIN")
stop_telem_button = Button(0.371, 0.729, 0.0488, 0.106, GREEN, bytearray(b'\x09\x09'), "Stop\nTelem", None, "BALLIN")
buttons.append(get_telem_button)
buttons.append(stop_telem_button)

indicators = {
    "CDH_CCLSM":indicator(0.651*width, 0.146*height, RED),
    "ADCS_CCLSM":indicator(0.651*width, 0.208*height, RED),
    "CAMERA_CCLSM":indicator(0.651*width, 0.271*height, RED),
    # "POWER_STATUS":indicator(800, 410, RED),
    # "ADCS_STATUS":indicator(800, 410, RED),
}

power_indicator = pygame.Rect(indicator_corner_anchor_x, indicator_corner_anchor_y, 0.01302*width, 0.02083*height)
adcs_indicator = pygame.Rect(indicator_corner_anchor_x, indicator_corner_anchor_y + 0.02604*height, 0.01302*width, 0.02083*height)
comms_indicator = pygame.Rect(indicator_corner_anchor_x, indicator_corner_anchor_y + 0.02604*2*height, 0.01302*width, 0.02083*height)

labels = [
    label(0.632*width, 0.172*height, "CDH\nPOWER", int(0.01*width)),
    label(0.632*width, 0.234*height, "ADCS\nPOWER", int(0.01*width)),
    # label(970, 285, "OTHER\nPOWER", 15),
    label(0.632*width, 0.296*height, "CAMERA\nPOWER", int(0.01*width)),
    label(0.0586*width, 0.281*height, "Messages from CDH", int(0.01*width)),
    label(0.638*width, 0.474*height, "Downlinked Image", int(0.01*width)),
    label(0.0521*width, 0.0677*height, "For Deployables", int(0.01*width)),
    label(0.335*width, 0.0677*height, "For ADCS", int(0.01*width)),
    label(0.626*width, 0.0677*height, "For Power", int(0.01*width)),
    label(indicator_corner_anchor_x+(0.0292*width), 0.2810*height, "Communication Status", int(0.01*width)),
    label(voltage_anchor_x+(0.026*width), 0.417*height, "Power Consumption", int(0.01*width)),
]

textbox = ScrollableTextbox(0.026*width, 0.292*height, 0.391*width, 0.208*height, WHITE, font)

def updateTextBoxSize():
    textbox.rect.width = textbox.rect.width * (width / old_width)
    textbox.rect.height = textbox.rect.height * (height / old_height)
    textbox.rect.x = textbox.rect.x * (width / old_width)
    textbox.rect.y = textbox.rect.y * (height / old_height)
    textbox.checkbox.rect.x = textbox.checkbox.rect.x * (width / old_width)
    textbox.checkbox.rect.y = textbox.checkbox.rect.y * (height / old_height)
    textbox.clearbox.rect.x = textbox.clearbox.rect.x * (width / old_width)
    textbox.clearbox.rect.y = textbox.clearbox.rect.y * (height / old_height)

def updateButtonSize():
    orbit_sim_control_button.rect.x = 0.026*width
    orbit_sim_control_button.rect.y = 0.51*height
    orbit_sim_control_button.rect.width = 0.34*width
    orbit_sim_control_button.rect.height = 0.03125*height
    connect_serial_button.rect.x = 0.625*width
    connect_serial_button.rect.y = 0.0052*height
    connect_serial_button.rect.width = 0.0814*width
    connect_serial_button.rect.height = 0.026*height
    connect_ps_button.rect.x = 0.837*width
    connect_ps_button.rect.y = 0.0052*height
    connect_ps_button.rect.width = 0.065*width
    connect_ps_button.rect.height = 0.026*height

def updateIndicatorSize():
    indicators["CDH_CCLSM"].rect.x = 0.651*width
    indicators["CDH_CCLSM"].rect.y = 0.146*height
    indicators["CDH_CCLSM"].rect.width = 0.013*width
    indicators["CDH_CCLSM"].rect.height = 0.0208*height
    indicators["ADCS_CCLSM"].rect.x = 0.651*width
    indicators["ADCS_CCLSM"].rect.y = 0.208*height
    indicators["ADCS_CCLSM"].rect.width = 0.013 * width
    indicators["ADCS_CCLSM"].rect.height = 0.0208 * height
    indicators["CAMERA_CCLSM"].rect.x = 0.651 * width
    indicators["CAMERA_CCLSM"].rect.y = 0.271*height
    indicators["CAMERA_CCLSM"].rect.width = 0.013 * width
    indicators["CAMERA_CCLSM"].rect.height = 0.0208 * height
    power_indicator.x = indicator_corner_anchor_x
    power_indicator.y = indicator_corner_anchor_y
    power_indicator.width = 0.01302*width
    power_indicator.height = 0.02083 * height
    adcs_indicator.x = indicator_corner_anchor_x
    adcs_indicator.y = indicator_corner_anchor_y + 0.02604 * height
    adcs_indicator.width = 0.01302 * width
    adcs_indicator.height = 0.02083 * height
    comms_indicator.x = indicator_corner_anchor_x
    comms_indicator.y = indicator_corner_anchor_y + 0.02604 * 2 * height
    comms_indicator.width = 0.01302 * width
    comms_indicator.height = 0.02083 * height

def updateLabels():
    labels[0].x = 0.632 * width
    labels[0].y = 0.172 * height
    labels[1].x = 0.632 * width
    labels[1].y = 0.234 * height
    labels[2].x = 0.632 * width
    labels[2].y = 0.296 * height
    labels[3].x = 0.0586 * width
    labels[3].y = 0.281 * height
    labels[4].x = 0.638 * width
    labels[4].y = 0.474 * height
    labels[5].x = 0.0521 * width
    labels[5].y = 0.0677 * height
    labels[6].x = 0.335 * width
    labels[6].y = 0.0677 * height
    labels[7].x = 0.626 * width
    labels[7].y = 0.0677 * height
    labels[8].x = indicator_corner_anchor_x + 45 * (width/old_width)
    labels[8].y = 0.281 * height
    labels[9].x = voltage_anchor_x + 40*(width/old_width)
    labels[9].y = 0.417 * height
...
def draw_image_black_box_outline():
    outline_rect = pygame.Rect(0.608*width, 0.484*height, 0.176*width, 0.365*height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

def draw_Deployables_buttons_black_box_outline():
    outline_rect = pygame.Rect(0.026 * width, 0.0781 * height, 0.273 * width, 0.1875 * height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

def draw_ADCS_buttons_black_box_outline():
    outline_rect = pygame.Rect(0.319 * width, 0.0781 * height, 0.273 * width, 0.1875 * height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

def draw_Power_buttons_black_box_outline():
    outline_rect = pygame.Rect(0.612 * width, 0.0781 * height, 0.176 * width, 0.25 * height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

def draw_Communication_Status_black_box_outline():
    outline_rect = pygame.Rect(indicator_corner_anchor_x-(0.0065*width), indicator_corner_anchor_y - (0.01*height), 0.111*width, 0.0938*height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

def draw_Power_Consumption_status_black_box_outline():
    outline_rect = pygame.Rect(voltage_anchor_x-(0.0065*width), voltage_anchor_y-(0.01*height), 0.111*width, 0.0625*height)
    pygame.draw.rect(window, BLACK, outline_rect, 2)

##################################################################################################
# Button to allow orbit simulation to control power supply
orbit_sim_control_button = Button(0.026, 0.51, 0.34, 0.03125, GREY, None, "Allow Orbit Sim. to Control Power Supply & Auto Telemetry Downlink", lambda: toggle_orbit_sim_control())

# Connect buttons for the serial COM ports
connect_serial_button = Button(0.625, 0.0052, 0.0814, 0.026, GREY, None, "Connect Radio", lambda: toggle_serial_connection())
connect_ps_button = Button(0.837, 0.0052, 0.065, 0.026, GREY, None, "Connect PS", lambda: toggle_ps_connection())

# buttons.append(orbit_sim_control_button)
# buttons.append(connect_serial_button)
# buttons.append(connect_ps_button)
...
def draw_dropdown_items(items, x, y, dropdown_width, dropdown_height, selected_item):
    for i, item in enumerate(items):
        item_rect = pygame.Rect(x, y + (i + 1) * dropdown_height, dropdown_width, dropdown_height)
...
def draw_com_port_dropdown():
    ...
    dropdown_rect = pygame.Rect(0.707*width, 0.0052*height, 0.0976*width, 0.026*height)
    ...
    window.blit(text_surface, (dropdown_rect.x + 0.00326*width, dropdown_rect.y + 0.0052*height))

    if dropdown_open:
        draw_dropdown_items(com_ports, 0.707*width, 0.0052*height, 0.0976*width, 0.026*height, selected_com_port)
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        if dropdown_rect.collidepoint(event.pos):
            ...
            for i, port in enumerate(com_ports):
                item_rect = pygame.Rect(0.707*width, 0.00326*width + (i + 1)*5*0.00326*width, 0.0976*width, 0.026*height)
...
def draw_ps_com_port_dropdown():
    ...
    dropdown_rect = pygame.Rect(0.902*width, 0.0052*height, 0.0976*width, 0.026*height)
    pygame.draw.rect(window, WHITE, dropdown_rect)
    pygame.draw.rect(window, BLACK, dropdown_rect, 2)
    ...
    if ps_dropdown_open:
        draw_dropdown_items(ps_com_ports, 0.902*width, 0.0052*height, 0.0976*width, 0.026*height, selected_ps_com_port)
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        if dropdown_rect.collidepoint(event.pos):
            ...
            for i, port in enumerate(ps_com_ports):
                item_rect = pygame.Rect(0.902*width, 0.00326*width + (i + 1)*5*0.00326*width, 0.0976*width, 0.026*height)
                if item_rect.collidepoint(event.pos):
...
def simulate_and_animate_day(semi_major_axis, eccentricity, inclination, raan, arg_periapsis, orbit_sim_speed, ground_station_latitude, ground_station_longitude):
    ...
    fig, ax = plt.subplots(figsize=(0.00325*1.1*width, 0.00325*1*height), subplot_kw={'projection': ccrs.PlateCarree()})
...
def update_oribt_animation():
    global frame, size, surf, width, height, old_width, old_height
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            for button in buttons:
                button.rect.width = int(button.rect.width * (width/old_width))
                button.rect.height = int(button.rect.height * (height/old_height))
                button.rect.x = int(button.rect.x * (width/old_width))
                button.rect.y = int(button.rect.y * (height/old_height))
            updateAnchors()
            updateTextBoxSize()
            updateIndicatorSize()
            updateLabels()
    ...
    if (toc1 - tic1 > 0.25):
        update_oribt_animation()
        tic1 = time.time()
    window.blit(surf, (0.01627*width, orbit_sim_control_button.rect.y + orbit_sim_control_button.rect.height + 0.00625*height))
    ...
    try:
        ...
        window.blit(image, (0.6185*width, 0.5*height))
    except:
        pass
